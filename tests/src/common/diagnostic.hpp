#pragma once
#include <string>
#include <string_view>
#include <vector>
#include <cstdint>
#include <iostream>
#include <algorithm>
#include "common/source_manager.hpp"

namespace femto {

struct DiagnosticItem {
    SourceSpan span;
    std::string message;
    bool is_error = true;
};

class Diagnostics {
public:
    explicit Diagnostics(const SourceManager& sm, bool print_to_stderr = true) 
        : sm_(sm), print_to_stderr_(print_to_stderr) {}

    void report_error(SourceSpan span, std::string_view message) {
        has_errors_ = true;
        items_.push_back(DiagnosticItem{ span, std::string(message), true });

        if (print_to_stderr_) {
            auto lc = sm_.get_line_col(span.start);
            std::cerr << "\033[1;31merror\033[0m: " << message 
                      << " \033[90m--> " << sm_.filename() << ":" << lc.line << ":" << lc.col << "\033[0m\n";
            std::cerr << "   \033[94m|\033[0m\n";
            std::cerr << (lc.line < 10 ? " " : "") << lc.line << " \033[94m|\033[0m " << lc.line_slice << "\n";
            
            uint32_t underline_len = (std::max)(1u, span.length);
            std::string underline = std::string(lc.col > 0 ? lc.col - 1 : 0, ' ') + 
                                    std::string(underline_len, '^');
            std::cerr << "   \033[94m|\033[0m \033[1;31m" << underline << "\033[0m\n";
        }
    }

    bool has_errors() const { return has_errors_; }
    const std::vector<DiagnosticItem>& items() const { return items_; }

private:
    const SourceManager& sm_;
    bool print_to_stderr_ = true;
    bool has_errors_ = false;
    std::vector<DiagnosticItem> items_;
};

} // namespace femto