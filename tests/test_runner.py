...
def main():
    ...
    if neg_tests:
        ...
        out_temp = build_dir / "temp_neg_bin.exe" if sys.platform == "win32" else build_dir / "temp_neg_bin"
        for idx, neg_file in enumerate(neg_tests, start=1):
            ...
            if out_temp.exists():
                out_temp.unlink()
            obj_temp = out_temp.with_suffix(".obj")
            if obj_temp.exists():
                obj_temp.unlink()
        print()
    ...
    out_bin = build_dir / "temp_test_bin.exe" if sys.platform == "win32" else build_dir / "temp_test_bin"
    ...
    for idx, test_file in enumerate(e2e_tests, start=1):
        ...
        if comp_res.returncode != 0:
            ...
            if comp_res.stdout:
                print(f"{GRAY}{comp_res.stdout.strip()}{RESET}")
            if comp_res.stderr:
        ...
        try:
            ...
            if out_bin.exists():
                out_bin.unlink()
            obj_temp = out_bin.with_suffix(".obj")
            if obj_temp.exists():
                obj_temp.unlink()
